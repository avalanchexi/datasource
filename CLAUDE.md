...
- `reserve_ratio` 结构化源仅允许 `official_china` PBoC；Trading Economics `cash-reserve-ratio` 是 7.50% 大行口径，已从结构化 provider 和搜索可信来源中屏蔽。PBoC 失败时应转 Tavily/manual，搜索链路遇到 `cash-reserve-ratio` URL 也应按错口径来源进入人工处理，并走上面的 PBoC quality-replacement 通道。
