...
- Stage4 报告路径固定 `reports/${DATE}-背景扫描120.md`；兼容入口：tests/scripts/generate_simple_report_test.py。
...
- **Stage2.5 注入** (`stage2_5_injector.py`) → `utils/`（`missing_items` canonical/兼容同步、`data_completion`、`trend_history_store`、`key_aliases`）
- **Stage3 Pring 分析** (`stage3_pring_analyzer.py`) → `calculators/`（`pring_analyzer` + `calculators/pring/` 子包、`economic_cycle_analyzer`、`fund_flow_calculator`、`technical_indicators`、`bond_calculator`）+ `trackers/policy_tracker` + gate 在 `utils/pipeline_gate`、`utils/policy_rules`、`utils/pipeline_quality_state`
- **Stage4 报告** (`stage4_report_generator.py`) → `generators/`（`report_generator`、`simple_report`）+ `comparators/`、`mappers/`、`analyzers/`
- **120 日背景扫描 agent** → `agents/background_scan/`
