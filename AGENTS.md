...
- Tavily search/extract 遇到 402/403/429/quota/rate-limit/payment 后，若已配置 `EXA_API_KEY`，Stage2 会将当前 run 从 `tavily_active` 切到 `exa_active`，用 Exa 重试当前失败任务，剩余任务也统一使用 Exa；不新增 quota probe，不重跑当日 Tavily。未配置 Exa 或 Exa 不可用时，缺口转 `manual_required` skeleton。
...
| Tavily quota/rate/payment | Tavily 返回 402/403/429、quota、rate-limit 或 payment 类错误 | 配置 `EXA_API_KEY` 时同轮切换 `exa_active`，用 Exa 重试当前失败任务并继续剩余任务；不要新增 quota probe 或重跑当日 Tavily，查看 `search_backend_final`、`tavily_to_exa_failover_count`、`exa_failover_*`、`exa_error_breakdown`。未配置 Exa 或 `exa_unavailable` 时转 Stage2.5 manual skeleton |
