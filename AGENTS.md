...
| Stage2 出现 SOCKS/代理错误 | 未通过 `run_clean.sh` 启动、显式 proxy mode 或宿主 VPN 改写代理环境 | 默认用 `bash run_clean.sh ...` 直连运行；确认未设置 `DATASOURCE_NETWORK_MODE=proxy`，再重跑 `bash run_preflight.sh`。需要代理时只用显式 proxy mode，并确认环境代理可连通 |
