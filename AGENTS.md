...
- 非日频货币指标（`reserve_ratio/mlf/reverse_repo/m0/m1/m2/tsf`）manual 必须提供真实 `as_of_date`（真实生效/操作日，可等于报告日）或 `report_period`（月度如 `2026-05`），不要依赖 `date=报告运行日`；gap_monitor 预填骨架已不再 seed 报告运行日。对 `reserve_ratio/reverse_repo/m0/m1/m2/tsf` 等非 MLF 非日频键，仅留 `date=报告运行日` 会被 `simple_report._pick_release_date` 设计性拒绝，报告「更新日期」列显示 N/A。`mlf` 多重价位/无统一利率/参考值展示有 Stage4 特例，可能回退显示 `date`；因此仍必须填真实 `as_of_date/report_period`，不要依赖报告运行日。
...
- Stage2 真实命中率优先看 `stage2_effective_hit_rate`；该指标包含 structured-provider 成功和搜索抽取成功，不包含 `skipped_existing`，也不包含 Stage2.5 manual 注入。分类型口径用嵌套 `stage2_category_breakdown`（每个 canonical category 含 `effective_success/search_success/structured_success/skipped_existing/manual_required/total`）取代旧的扁平 `success_by_category/search_success_by_category/total_by_category`；`fund_flow.effective_success` 只计 Stage2 写回（如 etf），Stage1 带来的 northbound/southbound 计入 `skipped_existing`。`stale强制刷新` 现按 `forced == 成功 + 跳过 + 待人工 + 其它` 完整切分，`成功` 含 structured_success。搜索链路命中率仍可看 `task_search_success/task_search_failed/search_success_rate_incremental`。
