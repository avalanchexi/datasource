...
def _has_quality_blocker(output, category, key, reason="estimated_not_allowed"):
    return {
        "category": category,
        "key": key,
        "reason": reason,
...
@pytest.mark.parametrize(
    "payload, expected",
    [
        ({"source_url": "https://www.pbc.gov.cn/official"}, "https://www.pbc.gov.cn/official"),
...
def test_manual_third_party_mlf_url_with_official_text_stays_estimated(tmp_path: Path, monkeypatch):
    _stub_trend_writes(monkeypatch)
    monkeypatch.chdir(tmp_path)
    run_dir = tmp_path / "data" / "runs" / "20260428"
    run_dir.mkdir(parents=True)
...
def test_manual_malformed_mlf_url_with_official_text_stays_estimated(tmp_path: Path, monkeypatch):
    _stub_trend_writes(monkeypatch)
    monkeypatch.chdir(tmp_path)
    run_dir = tmp_path / "data" / "runs" / "20260428"
    run_dir.mkdir(parents=True)
        ...
        manual_path,
        {
            "monetary_policy": {
                "mlf": {
                    ...
                    "unit": "%",
                    "date": "2026-04-25",
                    "source": "third-party estimate, reference 中国人民银行 policy",
                    "source_url": "https://",
                    "note": "malformed URL-like evidence should block issuer fallback",
                    "is_estimated": True,
                }
            }
        },
        ...
        manual_path,
        {
            "monetary_policy": {
                "mlf": {
                    ...
                    "unit": "%",
                    "date": "2026-04-25",
                    "source": "third-party https://evil.com?target=www.pbc.gov.cn reference 中国人民银行",
                    "note": "URL embedded in source is not an official host",
                    "is_estimated": True,
                }
            }
        },
...
def test_manual_mlf_malformed_source_token_blocks_official_fallback(tmp_path: Path, monkeypatch):
    _stub_trend_writes(monkeypatch)
    monkeypatch.chdir(tmp_path)
    run_dir = tmp_path / "data" / "runs" / "20260428"
    run_dir.mkdir(parents=True)
...
def test_manual_official_helper_http_trusted_explicit_source_url_blocks_official():
    assert injector._is_manual_official_value(
        ...
        {
            "policy_name": "MLF rate",
            "source_url": "http://www.pbc.gov.cn/path",
            "source": "PBOC official",
        },
    ) is False
...
def test_manual_official_helper_conflicting_explicit_source_urls_blocks_official():
    assert injector._is_manual_official_value(
        ...
        {
            "policy_name": "MLF rate",
            "source_url": "https://www.pbc.gov.cn/official,https://evil.com/conflict",
            "source": "PBOC official",
        },
    ) is False
...
def test_manual_third_party_usdcny_url_path_with_chinamoney_stays_estimated(tmp_path: Path, monkeypatch):
            ...
            "forex": [
                {
                    ...
                    "change_120d": 1.2,
                    "source": "第三方估算",
                    "source_url": "https://example.com/chinamoney/usdcny",
                    "note": "URL path contains chinamoney but domain is third-party",
                    "is_estimated": True,
                }
            ]
        },
    ...
    assert entry["is_estimated"] is True
    assert "manual_official_not_estimated" not in str(entry.get("note") or "")
    assert _has_quality_blocker(output, "forex", "USDCNY")


def test_manual_third_party_bcom_url_with_bloomberg_text_stays_estimated(tmp_path: Path, monkeypatch):
    _stub_trend_writes(monkeypatch)
    monkeypatch.chdir(tmp_path)
    run_dir = tmp_path / "data" / "runs" / "20260428"
    run_dir.mkdir(parents=True)
    ...
    _write_json(
        market_path,
        {
            "metadata": {"date": "2026-04-28", "missing_items": {"commodities": [{"key": "BCOM"}]}},
            "missing_items": ["BCOM"],
            "macro_indicators": {},
            "monetary_policy": {},
            "bonds": [],
            "forex": [],
            "commodities": [
                {"symbol": "BCOM", "name": "Bloomberg Commodity Index", "current_price": None, "source": "placeholder"}
            ],
            "stock_indices": [],
            "fund_flow": {},
        },
            ...
            "commodities": [
                {
                    ...
                    "current_price": 104.5,
                    "unit": "points",
                    "source": "第三方估算，参考 Bloomberg",
                    "source_url": "https://example.com/bloomberg/bcom",
                    "note": "Bloomberg token in third-party URL path",
                    "is_estimated": True,
                }
            ]
        },
    ...
    assert entry["is_estimated"] is True
    assert "manual_official_not_estimated" not in str(entry.get("note") or "")
    assert _has_quality_blocker(output, "commodities", "BCOM")
