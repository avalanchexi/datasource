...
- 口径:coverage >= 20% 记为 runtime_used(启发式,边缘值人工复核);零语句文件(空 __init__/纯 docstring)无运行时证据,按静态档分级
- 运行时证据仅来自离线回放 Stage2.5 -> Stage3 -> Stage4 report;Stage1/Stage2 专属模块最高只能定为 reachable_not_run
- 静态图覆盖 ast import + importlib.import_module 字面量/f-string 前缀(如 stage2_structured registry 动态加载);局限:__import__、运行期拼接字符串仍不可见
- 入口集:scripts/*.py 顶层共 23 个(不含 legacy/utility/archive 子目录),清单见 import_reachability.json entries
...
- `datasource.calculators.pring.leading_indicator`(src/datasource/calculators/pring/leading_indicator.py)— 引用方: 无引用
...
- `datasource.adapters`(src/datasource/adapters/__init__.py)— 引用方: 无引用
...
- `datasource.cache`(src/datasource/cache/__init__.py)— 引用方: 无引用
- `datasource.cache.sqlite_cache`(src/datasource/cache/sqlite_cache.py)— 引用方: scripts/stage2_unified_enhancer.py
- `datasource.calculators`(src/datasource/calculators/__init__.py)— 引用方: 无引用
- `datasource.calculators.pring`(src/datasource/calculators/pring/__init__.py)— 引用方: datasource.calculators.pring_analyzer
...
- `datasource.engines`(src/datasource/engines/__init__.py)— 引用方: 无引用
...
- `datasource.generators`(src/datasource/generators/__init__.py)— 引用方: 无引用
- `datasource.models`(src/datasource/models/__init__.py)— 引用方: 无引用
- `datasource.providers`(src/datasource/providers/__init__.py)— 引用方: 无引用
- `datasource.providers.stage2_structured`(src/datasource/providers/stage2_structured/__init__.py)— 引用方: datasource.providers.stage2_structured.registry, scripts/stage2_unified_enhancer.py
...
- `datasource.providers.stage2_structured.cdb_estimator`(src/datasource/providers/stage2_structured/cdb_estimator.py)— 引用方: datasource.providers.stage2_structured.registry
- `datasource.providers.stage2_structured.chinabond`(src/datasource/providers/stage2_structured/chinabond.py)— 引用方: datasource.providers.stage2_structured.cdb_estimator, datasource.providers.stage2_structured.registry
- `datasource.providers.stage2_structured.eastmoney_etf`(src/datasource/providers/stage2_structured/eastmoney_etf.py)— 引用方: datasource.providers.stage2_structured.registry
...
- `datasource.providers.stage2_structured.market_quote_pages`(src/datasource/providers/stage2_structured/market_quote_pages.py)— 引用方: datasource.providers.stage2_structured.registry
- `datasource.providers.stage2_structured.official_china`(src/datasource/providers/stage2_structured/official_china.py)— 引用方: datasource.providers.stage2_structured.registry
- `datasource.providers.stage2_structured.registry`(src/datasource/providers/stage2_structured/registry.py)— 引用方: datasource.providers.stage2_structured, datasource.providers.stage2_structured.registry
...
- `datasource.providers.stage2_structured.stooq`(src/datasource/providers/stage2_structured/stooq.py)— 引用方: datasource.providers.stage2_structured.registry
- `datasource.providers.stage2_structured.trading_economics`(src/datasource/providers/stage2_structured/trading_economics.py)— 引用方: datasource.providers.stage2_structured.registry
- `datasource.providers.stage2_structured.tushare_etf`(src/datasource/providers/stage2_structured/tushare_etf.py)— 引用方: datasource.providers.stage2_structured.registry
- `datasource.providers.stage2_structured.yahoo_finance`(src/datasource/providers/stage2_structured/yahoo_finance.py)— 引用方: datasource.providers.stage2_structured.registry
- `datasource.utils`(src/datasource/utils/__init__.py)— 引用方: 无引用
