#!/usr/bin/env python3
"""Archived helper that rewrites background scan tables using TuShare Pro data.

Current daily reports must be generated through Stage1 -> Stage4. This helper is
kept only for historical comparison and may only write under reports/archive/.
...
python scripts/utility/tushare_pro_report_patch.py \
    ...
    --output reports/archive/20250928tusharepro_archived.md
```
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Archived TuShare Pro report patch helper")
    parser.add_argument(
        "--run-archived",
        action="store_true",
    ...
    parser.add_argument(
        "--output",
        help="输出 Markdown 路径，默认 reports/archive/{date}tusharepro_archived.md",
    )
    parser.add_argument(
        "--window-days",
        type=int,
...
def main() -> None:
    ...
    if not args.run_archived:
        print(
            "[ARCHIVED] scripts/utility/tushare_pro_report_patch.py 已停用为归档工具。\n"
    ...
    output_path = Path(args.output) if args.output else PROJECT_ROOT / "reports" / "archive" / f"{end_date_compact}tusharepro_archived.md"
    archive_dir = (PROJECT_ROOT / "reports" / "archive").resolve()
    if archive_dir not in output_path.resolve().parents:
        print(
