...
from datasource.utils.missing_items import (
    flatten_missing_items as _shared_flatten_missing_items,
)
...
def _iter_estimated_entries(
    ...
    for key, entry in (market_payload.get("macro_indicators") or {}).items():
        if (
            ...
            and entry.get("current_value") not in (None, "N/A")
            and entry.get("is_estimated")
        ):
    ...
    for item in market_payload.get("bonds", []) or []:
        if (
            ...
            and item.get("current_yield") not in (None, 0, "N/A")
            and item.get("is_estimated")
        ):
            rows.append(("bonds", str(item.get("symbol")), item))
    for item in market_payload.get("forex", []) or []:
        if (
            isinstance(item, dict)
            and item.get("current_rate") not in (None, 0, "N/A")
            and item.get("is_estimated")
        ):
    ...
    for item in market_payload.get("commodities", []) or []:
        if (
            isinstance(item, dict)
            and item.get("current_price") not in (None, 0, "N/A")
            and item.get("is_estimated")
        ):
    ...
    *,
    ...
    for category, key, entry in _iter_estimated_entries(market_payload):
        ...
        allowed, reasons = is_estimated_allowlisted(
            category, key, entry, rules=policy_rules
        )
        if allowed:
...
def _append_non_blocking_warning(
    market_payload: Dict[str, Any], warning: Dict[str, Any]
) -> None:
            ...
            stale_items.append(
                {
                    ...
                    or "actual_period_behind_expected",
                }
            )
    return stale_items
...
def _policy_item_matches_live_blocker(
    item: Any, live_quality_blocker_keys: Set[str]
) -> bool:
    key = _gap_item_key(item)
    if not key:
        return False
...
def _policy_item_has_current_entry(
    market_payload: Dict[str, Any], item: Any
) -> bool:
...
def _require_data_completeness(
        ...
        blocks.append(
            GateBlock(
                ...
                [
                    (
                        "data_completeness="
                        f"{completeness:.3f} (<{min_completeness})"
                    ),
                    (
                        "run Stage2/Stage2.5 to refresh "
                        "market_data_complete.json before Stage3"
                    ),
                ],
            )
        )
    ...
    if quality_blockers:
        issue_lines = [
            _issue_label(item, market_payload) for item in quality_blockers
        ]
        blocks.append(GateBlock("unified_quality", issue_lines))

    if blocks:
        raise RuntimeError(
            format_gate_blocks("Stage3 阻断，以下问题需修复：", blocks)
        )
    return
...
def _is_allowlisted_gap_item(
    ...
    if not isinstance(entry, dict) or not entry.get("is_estimated"):
        return False
    allowed, _ = is_estimated_allowlisted(
        category, real_key, entry, rules=policy_rules
    )
    return allowed
...
def _resolve_gap_monitor_path(
    ...
    run_paths = build_run_paths_from_reference(
        payload=market_payload, fallback_to_today=True
    )
...
async def _run_analysis(
    ...
    critical_stale_keys = policy_rules.get(
    ...
    live_quality_state = build_pipeline_quality_state(
        ...
        policy_rules=_effective_policy_rules(
            ...
            critical_stale_keys=(
                critical_stale_keys
                if isinstance(critical_stale_keys, list)
                else None
            ),
        ),
        stage="stage3",
    ...
    if meta_date:
        policy_path = build_run_paths_from_reference(
            payload=market_payload, fallback_to_today=True
        ).policy_evaluation
        if policy_path.exists():
            try:
                ...
                for item in redlist_items:
                    ...
                    if _is_allowlisted_gap_item(
                        market_payload, item, policy_rules
                    ):
                        allowlisted_redlist.append(label)
                    elif _policy_item_matches_live_blocker(
                        item, live_quality_blocker_keys
                    ):
                        continue
                ...
                for key in allowlisted_redlist:
                    _append_non_blocking_warning(
                        market_payload,
                        {
                            ...
                            "key": key,
                            "message": (
                                "policy_evaluation redlist 白名单放行: "
                                f"{key}"
                            ),
                        },
                    )
                ...
                for item in stale_redlist:
                    ...
                    if not key:
                        continue
                    label = _item_label(item)
                    if _policy_item_matches_live_blocker(
                        item, live_quality_blocker_keys
                    ):
                        continue
                ...
                if policy_payload.get("block_stage3") and (
                    unresolved_redlist or unresolved_stale_redlist
                ):
                    policy_blockers.append(
                        (
                            f"redlist={unresolved_redlist}, "
                            f"stale_redlist={unresolved_stale_redlist}"
                        )
                    )

                if policy_payload.get("block_stage3") and (
                    diagnostic_redlist or diagnostic_stale_redlist
                ):
                    _append_non_blocking_warning(
                        market_payload,
                        {
                            ...
                            "key": "*",
                            "message": (
                                "policy_evaluation.json retained for "
                                "diagnostics only: "
                                f"redlist={diagnostic_redlist}, "
                                f"stale_redlist={diagnostic_stale_redlist}"
                            ),
                        },
                    )
    ...
    try:
        ...
        if allow_fallback:
            fallback_used = True
            print(
                f"[WARN] 数据完整性未达标，但 allow_fallback=True，继续运行。原因: {exc}"
            )
        else:
            completeness_blockers.extend(_message_items(completeness_error))

    # 3) gap monitor gate
    ai_websearch_flag = bool(
        market_payload.get("metadata", {}).get("ai_websearch_enhanced")
    )
            ...
            if _is_allowlisted_gap_item(market_payload, item, policy_rules):
                _append_non_blocking_warning(
                    market_payload,
                    {
                        ...
                        "key": label,
                        "message": (
                            "gap_monitor pending allowlist pass: "
                            f"{label}"
                        ),
                    },
                )
        ...
        if stale_gap_items:
            _append_non_blocking_warning(
                market_payload,
                {
                    ...
                    "key": "*",
                    "message": (
                        "gap_monitor retained for diagnostics only; "
                        "stale items ignored: "
        ...
        raise RuntimeError(
            format_gate_blocks(
                ...
                [
                    ...
                    GateBlock(
                        "completeness/unified_quality", completeness_blockers
                    ),
    ...
    print(
        f"[META] AI WebSearch 注入：{'已完成' if ai_websearch_flag else '未检测到'}"
    )
    print(
        f"[META] 宏观指标：{len(contract.macro_indicators)} 项，"
    ...
    pring_result.setdefault(
        "recommendation",
        pring_result.get("recommendation", "数据不足，无法生成建议"),
    )
    ...
    non_blocking_warnings = market_payload.get("metadata", {}).get(
    ...
    log_payload = {
        ...
        "stage": {
            ...
            "base_stage": pring_result.get("layer_3_pring_final", {}).get(
                "base_stage"
            ),
        },
        "data_sources": {
            "macro": pring_result.get("layer_1_inventory_cycle", {}).get(
                "data_source"
            ),
            "monetary": pring_result.get("layer_2_monetary_cycle", {}).get(
                "data_source"
            ),
            "asset": "market_price",
        },
        ...
        "warnings": (
            non_blocking_warnings
            if isinstance(non_blocking_warnings, list)
            else []
        ),
        "errors": [],
        "runtime_sec": round(runtime, 2),
    }
    log_path = build_run_paths_from_reference(
        payload=market_payload, fallback_to_today=True
    ).stage3_log
