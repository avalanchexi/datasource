...
class InventoryCycleLayer(BaseModel):
    ...
    indicators: Dict[str, Any]  # PPI, PMI, 工增, BDI, CPI (支持字符串或字典)
...
class MonetaryCycleLayer(BaseModel):
    ...
    indicators: Dict[str, Any]  # 降准, 降息, TSF, M2 (支持字符串或字典)
    websearch_needed: Dict[str, Any] = Field(default_factory=dict)  # 修复类型以匹配PringAnalyzer返回的字典
    ...
    class Config:
        json_schema_extra = {
            "example": {
                ...
                "monetary_score": 0.0,
                "indicators": {
                    "存款准备金率变化": {"weight": 0.30, "score": 0, "detail": "数据缺失，需WebSearch补充"},
                    "7天逆回购利率": {"weight": 0.30, "score": 0, "detail": "0/30分 (变化0bp)"},
                    "TSF增速": {"weight": 0.25, "score": 0, "detail": "数据缺失，需WebSearch补充"},
                    "M2增速": {"weight": 0.15, "score": 0, "detail": "数据缺失，需WebSearch补充"}
                },
                "websearch_needed": []
            }
        }
