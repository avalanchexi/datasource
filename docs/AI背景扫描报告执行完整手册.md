...
### 🔄 V2.1 严格数据源 + MCP增强自动化执行5阶段

| 阶段 | 阶段名称 | 时间 | 主要任务 | AI自动操作 |
|------|---------|------|----------|------------|
| 1️⃣ | 环境准备与严格验证 | 8分钟 | 检查配置、验证数据源和MCP工具 | TodoWrite、MCP验证 |
| 2️⃣ | 混合数据收集与MCP增强 | 12分钟 | 传统API+MCP并行收集 | 智能故障转移 |
| 3️⃣ | MCP智能数据补充 | 8分钟 | WebFetch/WebSearch实时补充 | MCP工具优先 |
| 4️⃣ | MCP增强报告优化 | 6分钟 | MCP数据标注与验证 | MultiEdit优化 |
| 5️⃣ | MCP质量验证与交付 | 6分钟 | MCP数据追溯与统计 | 质量保证 |

**V2.1总时间**: 35-45分钟 | **数据质量**: 显著提升 | **实时性**: ≤5分钟
...
**示例**: `执行背景扫描报告生成：20251014`

AI将自动按照5个阶段执行，总时间35-45分钟。
