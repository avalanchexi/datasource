...
**Goal:** 把 C4/C7 两次栽在"反向依赖/跨脚本 import 漏判"上的教训固化进重构方法论清单,记录 C7 后跨模块耦合审计的体检单(src→scripts 反向耦合=0),并顺手清一个 cosmetic docstring。**纯文档/注释改动,零代码逻辑、零测试逻辑。**

**Architecture:** 三处文档/注释编辑:① `optimization/20260610_refactor_plan/REFACTOR_PLAN.md` §11.2 plan 精准性清单加"反向依赖核查"项;② 同目录 `TODOS.md` 记审计体检单;③ `src/datasource/engines/stage2_5/schema_coercion.py` 一句 docstring 文案。无运行时行为变化。
...
## Task 1:REFACTOR_PLAN §11.2 加"反向依赖核查"项
...
```
- [ ] **Step 2** 校验:`wsl -e bash -lc 'cd /mnt/d/cursor/datasource && bash run_clean.sh python -m pytest tests/test_manual_template.py tests/test_stage4_docs.py -q 2>&1 | tail -3'`(REFACTOR_PLAN 非 runbook,预期不触发命令契约;确认仍绿)。commit `docs: add reverse-dependency check to refactor plan-precision checklist (C4/C7 lesson)`。
...
## Self-Review
- 覆盖:方法论加固 → Task 1;审计留痕 → Task 2;trivial docstring → Task 3;合入 → Task 4。✅
