...
6. Stage2.5 后仍有 `manual_required`：Stage3 继续阻断，不允许靠 `--allow-estimated` 绕过。
7. 检索有结果但抽取失败：记录为 `retrieval_hit_extract_failed`，不计入 Tavily 检索低命中。
8. 严格关键词或发布机构误伤：记录具体规则名和候选 query family，供 profile 回放调优。
...
11. 文档无需单测，但最终检查 `AGENTS.md` 与 `CLAUDE.md` 是否同步新增高频规则。
...
7. Stage2 命中率改进不新增联网搜索源，优先利用现有 Tavily 结果、日志回放和少量 profile 修正。
